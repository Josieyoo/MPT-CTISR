...
def before_calculate(hr_imgs, sr_imgs):
    hr_imgs = hr_imgs.data.squeeze().float().cpu().clamp_(0, 255).numpy()
    # hr_imgs = hr_imgs.cpu().numpy()
    hr_imgs = np.transpose(hr_imgs, (1, 2, 0))  # HWC
    hr_imgs = (hr_imgs * 255.0).round().astype(np.uint8)  # float32 to uint8
    # hr_imgs = np.squeeze(hr_imgs)
    # sr_imgs = sr_imgs.cpu().numpy()
    sr_imgs = sr_imgs.data.squeeze().float().cpu().clamp_(0, 255).numpy()
    sr_imgs = np.transpose(sr_imgs, (1, 2, 0))  # WHC
    sr_imgs = (sr_imgs * 255.0).round().astype(np.uint8)  # float32 to uint8
    # sr_imgs = np.squeeze(sr_imgs)

    return hr_imgs, sr_imgs